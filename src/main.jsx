  ...
  return (
    <div className="admin-shell">
      ...
      <main className="admin-main">
        ...
        {activeTab === 'rentals' && <Rentals rentals={filteredRentals} search={search} setSearch={setSearch} updateRentalStatus={updateRentalStatus} recordTestPayment={recordTestPayment} recordExtensionPayment={recordExtensionPayment} extensionRequests={extensionRequests} decideExtension={decideExtension} sendManualReminder={sendManualReminder} openDocument={openDocument} markDocument={markDocument} deleteDocument={deleteDocument} documentsByRentalId={documentsByRentalId} />}
...
function Dashboard({ dashboard, rentals, operationsQueue, documents, messages, reports, sendManualReminder, updateRentalStatus, openDocument, markDocument, documentsByRentalId }) {
  ...
  return <>
    ...
    <Panel title="Recent Rentals" eyebrow="Latest Activity">
      {recentRentals.map((r) => <RentalRow key={r.id} rental={r} updateRentalStatus={updateRentalStatus} sendManualReminder={sendManualReminder} rentalDocuments={documentsByRentalId[r.id] || []} openDocument={openDocument} markDocument={markDocument} />)}
    </Panel>
  </>;
}
...
function Rentals({ rentals, search, setSearch, updateRentalStatus, recordTestPayment, recordExtensionPayment, extensionRequests, decideExtension, sendManualReminder, openDocument, markDocument, deleteDocument, documentsByRentalId }) {
  ...
  return <>
    ...
    <Panel title="All Rentals" eyebrow="Reservations">
      ...
      <div className="table-list">{rentals.map((r) => <RentalRow key={r.id} rental={r} updateRentalStatus={updateRentalStatus} recordTestPayment={recordTestPayment} sendManualReminder={sendManualReminder} detailed rentalDocuments={documentsByRentalId[r.id] || []} openDocument={openDocument} markDocument={markDocument} deleteDocument={deleteDocument} />)}</div>
    </Panel>
  </>;
}
...
function RentalRow({ rental, updateRentalStatus, recordTestPayment, sendManualReminder, detailed, rentalDocuments = [], openDocument, markDocument, deleteDocument }) {
  const license = rentalDocuments.find((d) => d.document_type === 'license');
  ...
  const progressSteps = getRentalProgressSteps(rental, rentalDocuments);
  ...
  return <div className="data-row rental-row">
    <div>
      ...
      {detailed && <DocumentMiniList documents={rentalDocuments} openDocument={openDocument} markDocument={markDocument} deleteDocument={deleteDocument} />}
    </div>
    <div className="row-actions">
      <em>{prettyStatus(rental.status)}</em>
...
function buildOperationsQueue({ rentals, documents, messages, reports, extensionRequests = [] }) {
  ...
  rentals.forEach((rental) => {
    ...
    const hasLicense = rentalDocuments.some((document) => document.document_type === 'license');
    const hasInsurance = rentalDocuments.some((document) => document.document_type === 'insurance');
...
function getRentalProgressSteps(rental, rentalDocuments = []) {
  ...
  const steps = [
    ...
    { key: 'license', label: 'License', complete: hasLicense, detail: hasLicense ? 'Driver license uploaded' : 'Driver license missing' },
    { key: 'insurance', label: 'Insurance', complete: hasInsurance, detail: hasInsurance ? 'Insurance uploaded' : 'Insurance missing' },
