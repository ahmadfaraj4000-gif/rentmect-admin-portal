...
function App() {
  ...
  async function restoreCancelledRental(rental, paymentDueAt, reason) {
    if (!requireStaffPermission('rental.edit', 'restore cancelled unpaid reservations')) return false;
    const { data, error } = await supabase.rpc('admin_restore_cancelled_unpaid_rental', {
      p_rental_id: rental.id,
      p_payment_due_at: paymentDueAt,
      p_reason: reason.trim(),
    });
    ...
    notify(`Reservation restored. If it remains unpaid, it will cancel ${formatEasternDateTime(data?.payment_due_at || paymentDueAt)}.`, 'success');
    return data;
  }
  ...
  async function recordExternalRentalCharge(charge, payment) {
    ...
    notify(`${externalPaymentMethodLabel(payment.paymentMethod)} payment of ${money(charge.total_amount)} recorded for “${charge.name}”.`, 'success');
    return true;
  }
  ...
  async function createServiceFee(event) {
    ...
    notify('Internal charge template added. It is available from Rental Charges and will not affect public booking prices.', 'success');
  }
  ...
  async function toggleServiceFee(id, active) {
    ...
    notify(active ? 'Internal charge template activated.' : 'Internal charge template paused.', 'success');
  }
  ...
  async function deleteServiceFee(id) {
    ...
    notify('Internal charge template deleted.', 'success');
  }
  ...
  async function createManualStripePaymentLink({ rental, charge = null, extension = null, rentalBalance = false }) {
    ...
    if (error || data?.error || !data?.url) {
      let detail = data?.error || error?.message || 'The secure Stripe payment link could not be created.';
      try {
        const payload = await error?.context?.clone?.().json();
        detail = payload?.error || detail;
      } catch {
...
function Dashboard({ canViewFinancials = false, snapshot, dashboard, vehicles, rentals = [], maintenanceSchedules = [], emergencyExceptions = [], sendManualReminder, depositTasks = [], depositAllocations = [], onStripeRefund, onExternalRefund, onEscalate, onOpenRental }) {
  ...
  return <>
    <section className="metric-grid">
      ...
      {canViewFinancials && <Metric icon={Banknote} label="Month Revenue" value={money(snapshot?.month_revenue ?? dashboard.monthRevenue)} />}
      {canViewFinancials && <Metric icon={CreditCard} label="Active Deposits" value={money(snapshot?.active_deposits ?? dashboard.deposits)} />}
    </section>
    {canViewFinancials && <DepositActionPanel tasks={depositTasks} rentals={rentals} depositAllocations={depositAllocations} onStripeRefund={onStripeRefund} onExternalRefund={onExternalRefund} onEscalate={onEscalate} onOpenRental={onOpenRental} compact />}
...
function ExternalDepositReleaseModal({ rental, onCancel, onConfirm }) {
  ...
  return <div className="admin-modal-backdrop deposit-action-modal-backdrop" role="presentation"><form ref={dialogRef} className="admin-modal compact-modal deposit-action-modal" role="dialog" aria-modal="true" onSubmit={async (event) => {
    ...
    <header className="admin-modal-header"><div><small>Deposit resolution</small><strong>Record external deposit return</strong><span>{rental.profiles?.full_name || 'Customer'} • {money(rental.deposit_held_amount || rental.security_deposit)}</span></div><button type="button" className="admin-close-button" onClick={onCancel} disabled={saving} aria-label="Close deposit return dialog"><X size={18}/></button></header>
  ...
  return <div className="admin-modal-backdrop" role="presentation">
    <form ref={dialogRef} className="admin-modal availability-modal" role="dialog" aria-modal="true" aria-label="Calendar availability block" onSubmit={(event) => {
      ...
      <div className="admin-modal-header">
        ...
        <button type="button" className="admin-close-button" onClick={onCancel} disabled={modal.saving} aria-label="Close calendar block dialog"><X size={18}/></button>
      </div>
...
function CustomerContactModal({ profile, rentals, emailTemplates = [], smsTemplates = [], notify, initialTemplateKey = '', charge = null, extension = null, createPaymentLink = null, onClose }) {
  ...
  const [paymentLink, setPaymentLink] = useState({ loading: Boolean(createPaymentLink), url: '', error: '', copied: false });
  const paymentLinkRequest = useRef(0);
  const createPaymentLinkRef = useRef(createPaymentLink);
  ...
  useEffect(() => {
    const paymentLinkFactory = createPaymentLinkRef.current;
    if (!paymentLinkFactory) return;
    const requestId = paymentLinkRequest.current + 1;
    paymentLinkRequest.current = requestId;
    setPaymentLink({ loading: true, url: '', error: '', copied: false });
    Promise.resolve(paymentLinkFactory())
      .then((url) => {
        if (paymentLinkRequest.current !== requestId) return;
        if (!url) throw new Error('Stripe did not return a checkout URL.');
        setPaymentLink({ loading: false, url, error: '', copied: false });
      })
      .catch((linkError) => {
        if (paymentLinkRequest.current !== requestId) return;
        setPaymentLink({ loading: false, url: '', error: linkError instanceof Error ? linkError.message : 'The Stripe checkout URL could not be created.', copied: false });
      });
    return () => {
      if (paymentLinkRequest.current === requestId) paymentLinkRequest.current += 1;
    };
  }, []);
  ...
  async function copyPaymentLink() {
    ...
    try {
      ...
      setPaymentLink((current) => ({ ...current, copied: true }));
      notify('Stripe checkout link copied.', 'success');
    } catch {
      setPaymentLink((current) => ({ ...current, error: 'Copy was blocked by the browser. Select the URL and copy it manually.' }));
    }
  }
  ...
  return <div className="admin-modal-backdrop customer-contact-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <form ref={dialogRef} className="admin-modal customer-contact-modal" role="dialog" aria-modal="true" aria-label={`Message ${profile.full_name || 'customer'}`} onSubmit={sendMessage}>
      ...
      <div className="customer-contact-body">
        ...
        {createPaymentLink && <section className={`stripe-link-copy-panel ${paymentLink.error ? 'has-error' : ''}`} aria-label="Stripe checkout link">
          <div className="stripe-link-copy-heading"><div><CreditCard size={17}/><span><strong>Stripe checkout URL</strong><small>Copy this link to send it yourself. Nothing is sent automatically.</small></span></div>{paymentLink.loading && <em>Creating secure link…</em>}</div>
          {paymentLink.url && <div className="stripe-link-copy-row"><input aria-label="Stripe checkout URL" value={paymentLink.url} readOnly onFocus={(event) => event.currentTarget.select()} /><button type="button" className="stripe-link-copy-button" onClick={copyPaymentLink}><Copy size={15}/>{paymentLink.copied ? 'Copied' : 'Copy link'}</button><a href={paymentLink.url} target="_blank" rel="noreferrer"><ExternalLink size={15}/> Open link</a></div>}
          {paymentLink.error && <div className="stripe-link-copy-error"><span>{paymentLink.error}</span><button type="button" onClick={() => {
            const requestId = paymentLinkRequest.current + 1;
            paymentLinkRequest.current = requestId;
            setPaymentLink({ loading: true, url: '', error: '', copied: false });
            Promise.resolve(createPaymentLinkRef.current?.()).then((url) => {
              if (paymentLinkRequest.current === requestId) setPaymentLink({ loading: false, url: url || '', error: url ? '' : 'Stripe did not return a checkout URL.', copied: false });
            }).catch((linkError) => {
              if (paymentLinkRequest.current === requestId) setPaymentLink({ loading: false, url: '', error: linkError instanceof Error ? linkError.message : 'The Stripe checkout URL could not be created.', copied: false });
            });
...
function ManualBooking({ manualBookingForm, setManualBookingForm, profiles, customerDirectoryState, refreshCustomerDirectory, vehicles, rentals, pendingBookings = [], availabilityBlocks, under25Pricing, bookingPolicy = DEFAULT_BOOKING_POLICY, createManualBooking, submitting }) {
  const [customerSearch, setCustomerSearch] = useState('');
  ...
  const taxTotal = rentalTotal * CT_TAX_RATE;
    ...
    <div className="manual-booking-sidebar">
      <aside className="booking-summary-card">
        ...
        <dl>
          ...
          <div><dt>Total due</dt><dd>{selectedVehicle ? money(rentalTotal + taxTotal + deposit) : '—'}</dd></div>
        </dl>
...
function SettingsTab({
  ...
  return <section className="settings-grid">
    ...
    {settingsSection === 'pricing' && <Panel title="Internal Charge Templates" eyebrow="Rental Billing">
      <p className="muted">These templates are internal only. Active templates appear in each reservation’s Rental Charges section and are never included in public prices, checkout, or new-booking totals.</p>
      <form className="portal-form settings-form" onSubmit={createServiceFee}>
        <input placeholder="Template name e.g. speeding, reactivation, cleaning" maxLength="60" value={serviceFeeForm.name} onChange={(event) => updateFee('name', event.target.value)} />
        <div className="form-row">
        ...
        <textarea placeholder="Optional internal description copied onto the rental charge when applied" maxLength="240" value={serviceFeeForm.description} onChange={(event) => updateFee('description', event.target.value)} />
        <div className="form-row compact">
          ...
          <label className="checkbox-pill"><input type="checkbox" checked={serviceFeeForm.active} onChange={(event) => updateFee('active', event.target.checked)} /> Available in Rental Charges</label>
        </div>
        <button className="primary-btn"><Plus size={17}/> Add Internal Template</button>
      </form>

      <div className="settings-list">
        {serviceFees.length === 0 && <p className="muted">No internal charge templates yet.</p>}
      ...
      <div className="rental-card-command">
        ...
        <div className="rental-card-secondary-actions">
          ...
          <details className="rental-overflow-menu">
            ...
            <div>
              <span className="rental-overflow-menu-label">Reservation actions</span>
              {canRestoreCancelledRental && <button type="button" className="restore-rental-action" onClick={() => setRestoreModalOpen(true)}><History size={14}/> Restore rental</button>}
              ...
              {!canRestoreCancelledRental && !canAdjustPaymentDeadline && !canCreateEmergencyException && !canCancel && <span>No additional actions</span>}
            </div>
          </details>
        </div>
      </div>
    ...
    <details className="rental-card-activity" open={activityNeedsAttention || undefined}>
      ...
      {restoreModalOpen && <RestoreCancelledRentalModal
        ...
        onConfirm={async (paymentDueAt, reason) => {
          const restored = await restoreCancelledRental?.(rental, paymentDueAt, reason);
          if (restored) setRestoreModalOpen(false);
          return restored;
        }}
      />}
      ...
      />, document.body)}
      {contactModal && <CustomerContactModal profile={rental.profiles || { id: rental.user_id, email: rental.user_email }} rentals={[rental]} emailTemplates={emailTemplates} smsTemplates={smsTemplates} notify={notify} initialTemplateKey={contactModal.extension ? 'manual_extension_payment_due' : contactModal.charge ? 'manual_additional_charge_due' : contactModal.initialTemplateKey || ''} charge={contactModal.charge || null} extension={contactModal.extension || null} createPaymentLink={(contactModal.charge || contactModal.extension) && createManualStripePaymentLink ? () => createManualStripePaymentLink({ rental, charge: contactModal.charge || null, extension: contactModal.extension || null, rentalBalance: Boolean(contactModal.rentalBalance) }) : null} onClose={() => setContactModal(null)} />}
...
function RestoreCancelledRentalModal({ rental, onCancel, onConfirm }) {
  ...
  async function submit(event) {
    ...
    setError('');
    if (!deadline) {
      setError('Choose a cancellation deadline before restoring this reservation.');
      return;
    }
    const paymentDueAt = easternDateTimeInputToIso(deadline);
    if (!paymentDueAt || new Date(paymentDueAt).getTime() < minimumDeadline.getTime()) {
      setError('Choose a cancellation deadline at least five minutes from now in Eastern Time.');
      return;
    }
    if (new Date(paymentDueAt).getTime() > maximumDeadline.getTime() + 60_000) {
      setError('The cancellation deadline cannot be more than seven days away.');
      return;
    }
    if (reason.trim().length < 5) {
      setError('Enter a restore reason of at least five characters for the audit log.');
      return;
    }
    setSaving(true);
    const restored = await onConfirm(paymentDueAt, reason.trim());
    if (!restored) setSaving(false);
  }

  return <div className="admin-modal-backdrop restore-rental-backdrop" role="presentation">
    <form ref={dialogRef} className="admin-modal rental-deadline-modal restore-rental-modal" role="alertdialog" aria-modal="true" aria-label="Restore cancelled reservation with a required cancellation deadline" onSubmit={submit}>
      <header className="admin-modal-header">
        <History size={22}/>
        <div><small>Cancelled unpaid reservation</small><strong>Restore reservation</strong><span>{rental.profiles?.full_name || 'Customer'} • {rental.vehicles?.name || 'Vehicle'}</span></div>
      ...
      <div className="rental-deadline-body">
        <div className="restore-deadline-warning" role="alert">
          <AlertTriangle size={20}/>
          <span><strong>A cancellation time is mandatory.</strong> This reservation will not be restored until you choose exactly when it must cancel again if payment is still missing.</span>
        </div>
        <div className="deadline-current-summary">
          <span><strong>Cancelled</strong>{rental.cancelled_at ? formatEasternDateTime(rental.cancelled_at) : 'Cancellation time not recorded'}</span>
          <span><strong>Previous reason</strong>{rental.cancellation_reason || 'No cancellation reason recorded'}</span>
        </div>
        <div className="deadline-quick-actions" role="group" aria-label="Required cancellation deadline choices">
        ...
        <label><span>Cancel again if still unpaid — Eastern Time</span><input type="datetime-local" required min={formatEasternDateTimeInput(minimumDeadline)} max={formatEasternDateTimeInput(maximumDeadline)} value={deadline} onChange={(event) => setDeadline(event.target.value)}/><small>No time is preselected. The administrator must choose one.</small></label>
        <label><span>Reason for restoring this reservation</span><textarea required minLength="5" maxLength="500" value={reason} onChange={(event) => setReason(limitText(event.target.value, 500))} placeholder="Why is this cancelled reservation being restored?"/></label>
        {error && <p className="form-error" role="alert">{error}</p>}
      </div>
      <footer className="modal-actions">
        <button type="button" className="secondary-btn" onClick={onCancel} disabled={saving}>Keep Cancelled</button>
        <button type="submit" className="primary-btn restore-rental-confirm" disabled={saving || !deadline || reason.trim().length < 5}>{saving ? 'Restoring…' : 'Restore With This Deadline'}</button>
      </footer>
    </form>
  </div>;
}
...
function RentalChargeManager({ rental, charges = [], serviceFees = [], addRentalCharge, waiveRentalCharge, chargeRentalSavedCard, recordExternalRentalCharge, sendPaymentLink, notify, compact = false }) {
  ...
  return <div className={`rental-charge-manager ${compact ? 'compact' : ''}`}>
    ...
    {open && <form className="portal-form rental-charge-form" onSubmit={submit}>
      ...
      <button className="approve" disabled={saving}>{saving ? 'Adding…' : 'Apply charge to this rental'}</button>
    </form>}
    {externalCharge && <ChargeExternalPaymentModal rental={rental} charge={externalCharge} onCancel={() => setExternalCharge(null)} onConfirm={async (payment) => {
      const recorded = await recordExternalRentalCharge?.(externalCharge, payment);
      if (recorded) setExternalCharge(null);
      return recorded;
    }} />}
  </div>;
}

function ChargeExternalPaymentModal({ rental, charge, onCancel, onConfirm }) {
  const dialogRef = useDialogFocus(onCancel);
  const [form, setForm] = useState({ paymentMethod: 'cash', reference: '', confirmed: false });
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState('');
  ...
  async function submit(event) {
    ...
    if (!form.confirmed) {
      setError('Confirm that the payment cleared before recording it.');
      return;
    }
    setSaving(true);
    const recorded = await onConfirm?.(form);
    setSaving(false);
    if (!recorded) setError('The payment was not recorded. Review the message above and try again.');
  }

  return createPortal(<div className="admin-modal-backdrop external-charge-payment-backdrop">
    <form ref={dialogRef} className="admin-modal external-charge-payment-modal" role="dialog" aria-modal="true" aria-labelledby={`external-charge-title-${charge.id}`} onSubmit={submit}>
      <header className="admin-modal-header">
        <Banknote size={28}/>
        <div><strong id={`external-charge-title-${charge.id}`}>Record Cash / External Payment</strong><span>{rental.vehicles?.name || 'Vehicle'} • {charge.name}</span></div>
        <button type="button" className="admin-close-button" onClick={onCancel} disabled={saving} aria-label="Close external charge payment dialog"><X size={18}/></button>
      </header>
      <div className="external-payment-total"><span>Manual charge received</span><strong>{money(charge.total_amount)}</strong></div>
      <label className="modal-field"><span>Payment type</span><select value={form.paymentMethod} onChange={(event) => setForm((current) => ({ ...current, paymentMethod: event.target.value }))} required>
        <option value="cash">Cash</option><option value="terminal">Card terminal</option><option value="card">Card collected externally</option><option value="cash_app">Cash App</option><option value="bank_transfer">Bank transfer</option><option value="other">Other external payment</option>
      </select></label>
      <label className="modal-field"><span>Receipt or reference (optional)</span><input value={form.reference} maxLength="120" onChange={(event) => setForm((current) => ({ ...current, reference: limitText(event.target.value, 120) }))} placeholder="Receipt number or payment note" /></label>
      <div className="override-warning"><strong>This settles only “{charge.name}”</strong><span>The payment will appear in this rental's Payments Received ledger and the charge will no longer be due.</span></div>
      <label className="external-payment-confirmation"><input type="checkbox" checked={form.confirmed} onChange={(event) => setForm((current) => ({ ...current, confirmed: event.target.checked }))}/><span>I confirm {money(charge.total_amount)} was received and cleared.</span></label>
      {error && <small className="form-error" role="alert">{error}</small>}
      <div className="modal-actions"><button type="button" onClick={onCancel} disabled={saving}>Cancel</button><button className="approve" disabled={saving || !form.confirmed}>{saving ? 'Recording…' : `Record ${money(charge.total_amount)}`}</button></div>
    </form>
  </div>, document.body);
}
  ...
  return <div className="admin-modal-backdrop price-confirmation-backdrop" role="presentation" onMouseDown={(event) => {
    ...
    >
      <div className={`admin-modal-header${isSingleDigit ? ' danger' : ''}`}>
        ...
        <button type="button" className="admin-close-button" onClick={onCancel} disabled={confirming} aria-label="Close price confirmation"><X size={18}/></button>
      </div>
  ...
  return <div className="admin-modal-backdrop" role="presentation">
    <div ref={dialogRef} className="admin-modal" role="dialog" aria-modal="true" aria-label={title} tabIndex="-1">
      <div className="admin-modal-header">
        ...
        <button type="button" className="admin-close-button" onClick={onCancel} aria-label="Close override dialog"><X size={18}/></button>
      </div>
  ...
  return <div className="admin-modal-backdrop" role="presentation">
    <form ref={dialogRef} className="admin-modal" role="dialog" aria-modal="true" aria-label={override ? 'Override Pickup' : 'Mark Vehicle Picked Up'} onSubmit={submit}>
      <div className="admin-modal-header">
        ...
        <button type="button" className="admin-close-button" onClick={onCancel} aria-label="Close pickup dialog"><X size={18}/></button>
      </div>
      {override && <>
        <div className="override-warning">
          <strong>Automatic checklist is incomplete.</strong>
...
function buildRentalPaymentHistory(rental, rentalPayments = [], rentalCharges = [], extensionRequests = [], initialPaymentTotal = 0, externalPaymentActions = [], rentalRefunds = [], depositAllocations = []) {
  ...
  const canonicalPayments = rentalPayments
    ...
    .map((payment) => {
      const provider = String(payment.payment_provider || '').toLowerCase() === 'stripe' ? 'stripe' : 'external';
      const paymentType = String(payment.payment_type || 'rental').toLowerCase();
      const totalAmount = Number(payment.total_amount || 0)
        || Number(payment.rental_amount || 0) + Number(payment.tax_amount || 0) + Number(payment.deposit_amount || 0);
      return {
        id: `ledger-payment-${payment.id}`,
        amount: totalAmount,
        date: payment.paid_at || payment.updated_at || payment.created_at,
        provider,
        method: provider === 'external' ? externalPaymentMethodLabel(payment.external_payment_method) : '',
        label: paymentType.includes('extension')
          ? 'Rental extension'
  ...
  const canonicalCheckoutSessions = new Set(canonicalPayments.map((payment) => payment.checkoutSessionId).filter(Boolean));
  const matchesCanonicalPayment = (source) => (
    (source.stripe_payment_intent_id && canonicalPaymentIntents.has(source.stripe_payment_intent_id))
    || (source.stripe_checkout_session_id && canonicalCheckoutSessions.has(source.stripe_checkout_session_id))
  );
  const payments = [...canonicalPayments, ...rentalCharges
  ...
  extensionRequests
    .filter((request) => request.status === 'activated' || String(request.payment_status || '').toLowerCase() === 'paid')
    .filter((request) => !matchesCanonicalPayment(request))
    .forEach((request) => {
      const provider = String(request.payment_provider || '').toLowerCase() === 'stripe' ? 'stripe' : 'external';
      payments.push({
        id: `extension-payment-${request.id}`,
        amount: Number(request.payment_amount_cents || 0) > 0
          ? Number(request.payment_amount_cents) / 100
          : Number(request.extension_total_amount || 0),
        date: request.paid_at || request.updated_at || request.created_at,
        provider,
        method: provider === 'external' ? externalPaymentMethodLabel(request.external_payment_method) : '',
        label: request.request_kind === 'switch_car_continuation' ? 'Vehicle switch' : 'Rental extension',
      });
    });

  const hasCanonicalRentalPayment = canonicalPayments.some((payment) => !payment.paymentType.includes('extension'));
  if (rental?.paid_at && !hasCanonicalRentalPayment && !matchesCanonicalPayment(rental)) {
  ...
  return createPortal(<div className="admin-modal-backdrop external-payment-backdrop">
    <form ref={dialogRef} className="admin-modal external-payment-modal" role="dialog" aria-modal="true" aria-labelledby={`external-payment-title-${rental.id}`} onSubmit={submit}>
      <header className="admin-modal-header">
        ...
        <button type="button" className="admin-close-button" onClick={onCancel} disabled={saving} aria-label="Close external payment dialog"><X size={18}/></button>
      </header>
...
function ReturnCompletionPanel({ rental, onCancel, onComplete }) {
  ...
  <form ref={dialogRef} className="admin-modal return-completion-panel return-completion-modal" role="dialog" aria-modal="true" aria-labelledby={`return-completion-title-${rental.id}`} onSubmit={submit}>
    <header className="admin-modal-header">
      ...
      <button type="button" className="admin-close-button" onClick={onCancel} disabled={saving} aria-label="Close return completion dialog"><X size={18}/></button>
    </header>
