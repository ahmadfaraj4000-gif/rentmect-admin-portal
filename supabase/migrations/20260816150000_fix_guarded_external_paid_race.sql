...
-- Check the caller's observed collection state before treating a paid row as
-- idempotent. Otherwise a concurrent Stripe success could be mislabeled as an
-- external payment by the Edge Function's audit log.
